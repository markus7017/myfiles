...
The device profile is selected in the Shelly App/device settings and controls which channel groups are active: `rgb`, `rgbw` or `light`.
Groups `control`, `color` and `white` are only populated in `rgb`/`rgbw` profile.
Groups `channel1`..`channel4` are only populated in `light` profile, each channel having its own independent input.

| Group    | Channel      | Type     | read-only | Description                                                                     |
| -------- | ------------ | -------- | --------- | ------------------------------------------------------------------------------- |
| control  | power        | Switch   | r/w       | Switch light ON/OFF (rgb/rgbw profile)                                          |
|          | input        | Switch   | yes       | ON: Input is powered, see General Notes on Channels                             |
|          | button       | Trigger  | yes       | Event trigger, see section Button Events                                        |
|          | autoOn       | Number   | r/w       | Sets a  timer to turn the device ON after every OFF command; in seconds         |
|          | autoOff      | Number   | r/w       | Sets a  timer to turn the device OFF after every ON command; in seconds         |
|          | timerActive  | Switch   | yes       | ON: An auto-on/off timer is active                                              |
| color    | hsb          | HSB      | r/w       | Represents the color picker (HSBType); control r/g/b, not white                 |
|          | full         | String   | r/w       | Set Red / Green / Blue / White mode and switch mode                             |
|          |              |          |           | Valid settings: "red", "green", "blue", "white" or "r,g,b,w"                    |
|          | red          | Dimmer   | r/w       | Red brightness: 0..100% or 0..255 (control only the red channel)                |
|          | green        | Dimmer   | r/w       | Green brightness: 0..100% or 0..255 (control only the green channel)            |
|          | blue         | Dimmer   | r/w       | Blue brightness: 0..100% or 0..255 (control only the blue channel)              |
|          | white        | Dimmer   | r/w       | White brightness: 0..100% or 0..255 (control only the white channel, rgbw only) |
| white    | brightness   | Dimmer   | r/w       | Overall brightness/gain: 0..100% (rgb/rgbw profile)                             |
| channel1 | brightness   | Dimmer   | r/w       | Channel 1: Brightness: 0..100, control power state with ON/OFF                  |
|          | input        | Switch   | yes       | ON: Input for channel 1 is powered                                              |
|          | button       | Trigger  | yes       | Event trigger, see section Button Events                                        |
|          | autoOn       | Number   | r/w       | Sets a  timer to turn the device ON after every OFF command; in seconds         |
|          | autoOff      | Number   | r/w       | Sets a  timer to turn the device OFF after every ON command; in seconds         |
|          | timerActive  | Switch   | yes       | ON: An auto-on/off timer is active                                              |
| channel2 |              |          |           | Same as channel1, but for Channel 2                                             |
| channel3 |              |          |           | Same as channel1, but for Channel 3                                             |
| channel4 |              |          |           | Same as channel1, but for Channel 4                                             |
| meter    | currentWatts | Number   | yes       | Current power consumption in Watts (all channels)                               |
|          | lastPower1   | Number   | yes       | Average power consumption during the previous minute                            |
|          | totalKWH     | Number   | yes       | Total energy consumption in kWh since the device powered up (resets on restart) |
|          | lastUpdate   | DateTime | yes       | Timestamp of the last measurement                                               |
| device   | alarm        | Trigger  | yes       | Fires when device reports an alarm/error condition                              |
|          | wifiSignal   | Number   | yes       | Wi-Fi signal strength (0..4)                                                    |

### Shelly Pro RGBWW PM (thing-type: shellyprorgbwwpm)

Channel layout is the same as for the Shelly Plus RGBW PM, but with a 5th channel (`channel5`) for `light` profile and 2 additional profiles: `rgbcct` (RGB + CCT) and `rgbx2light` (RGB + 2 independent Light channels).
In these 2 hybrid profiles only the RGB portion is currently exposed as openHAB channels (group `control`/`color`/`white`); the CCT/extra Light components (which use their own outputs and, for `rgbx2light`, their own inputs) are not yet mapped to channels.
Use profile `rgb`, `rgbw` or `light` for full channel coverage.
...
| channel3 |              |          |           | Same as channel1, but for Channel 3                                             |
| channel4 |              |          |           | Same as channel1, but for Channel 4                                             |
| channel5 |              |          |           | Same as channel1, but for Channel 5                                             |
| meter    | currentWatts | Number   | yes       | Current power consumption in Watts (all channels)                               |
|          | lastPower1   | Number   | yes       | Average power consumption during the previous minute                            |
|          | totalKWH     | Number   | yes       | Total energy consumption in kWh since the device powered up (resets on restart) |
|          | lastUpdate   | DateTime | yes       | Timestamp of the last measurement                                               |
...
Channels in group `color`/`white` and groups `channel1`..`channelN` apply depending on the selected profile - they are not active at the same time.
