...
### Average Energy (2/3 Minutes)

`energyAvg2Min` and `energyAvg3Min` report the average energy (in Wh) over the previous 2 and 3 minutes respectively.
They are computed by the binding from the same per-minute samples used for `energyAvg1Min`, so they are available on every device that also exposes `energyAvg1Min`.
These channels smooth out short spikes and dips, which is useful for automations that should react to a sustained load change rather than a single noisy sample.
...
- Single- and dual-meter devices (relay-PM, EM/EM1 clamp meters, EM-50, EM Mini) expose `resetTotals` per meter group (`meter#resetTotals`, `meter1#resetTotals`, ...), resetting only that meter's counters.
- Three-phase 3EM devices (Gen1 3EM, Plus 3EM-63, Pro 3EM) do not support resetting a single phase; `resetTotals` is exposed once in the common `device` group and resets all phases together.
...
|         | energyAvg2Min | Number   | yes       | Average energy over the previous 2 minutes (Wh)                           |
|         | energyAvg3Min | Number   | yes       | Average energy over the previous 3 minutes (Wh)                           |
